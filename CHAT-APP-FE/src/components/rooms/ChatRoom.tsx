...
const ChatRoom = ({message, setMessage, sendMessage, chatRoomId, messageArray, leaveRoom, userId, DeleteMessage, userCount}: ChatPropsInterface ) => {
  ...
  function scrollToBottom() {
    ...
    if(isNearBottom || lastMessage.userId === userId){
  ...
  return (
    <>
    <nav className="h-[8%] w-full bg-neutral-950/90  text-white px-10 flex items-center justify-between font-sans-flex border-b border-neutral-500">
   ...
   <div className="h-[92%] selection:bg-white/30 max-w-4xl mx-auto relative rounded-2xl p-2 bg-bg py-4 flex flex-col gap-1 font-mono items-end overflow-hidden">
          ...
          <div ref={chatContainerRef} className="relative text-white h-full w-full no-scrollbar py-2 pt-4 px-1 overflow-y-auto flex flex-col items-center gap-4 mask-t-from-97% mask-b-from-95%  z-5 overflow-x-hidden">
            ...
            <span className="absolute text-neutral-400 opacity-70  flex-col flex gap-2 top-70 text-center text-lg font-sans-flex left-1/2 -translate-x-[50%] -translate-y-[50%]">
              <h1 className="text-2xl tracking-wider italic">
                No messages yet !
              </h1>
              <h6 className="text-sm tracking-widest font-thin ">
                Start your conversation right away by sending the message below.
              </h6>
            </span> : 
            messageArray.map((message, idx) => { 
              if(message.type === 'system'){
                return <div className=" w-full text-sm bg-neutral-600/50 italic text-neutral-100 text-center font-sans-flex font-thin tracking-wider mask-r-from-20% mask-l-from-20% ">
          ...
          <form onSubmit={(e) => sendMessage(e)} className="flex selection:bg-neutral-50 h-18 text-lg bg-neutral-500/50 w-full mx-auto gap-2 shadow-aceternity-dark  p-1 rounded-full">
            <input 
              ...
              className="flex-1 h-full rounded-full py-2 px-3 pl-5
              placeholder:text-neutral-500/50 hover:placeholder:text-neutral-500 text-neutral-300 outline-none duration-300 transition-all" 
            />
            <button disabled={checkValueRef.current?.value.trim() === ''} className={`h-full w-[20%] disabled:bg-accent/40 text-neutral-800 bg-accent/85 disabled:cursor-not-allowed  transition-all 
              duration-100 text-lg font-sans-flex inset-shadow-aceternity-dark rounded-full hover:shadow-aceternity-dark active:scale-95 disabled:active:scale-100  cursor-pointer`}> 
              Send
            </button>
          </form>
